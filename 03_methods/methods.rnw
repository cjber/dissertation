...
Total length between nodes calculated with euclidean distance:

\[
\begin{aligned}
tot_{len} = \sqrt{(Nx2 - nx1)^2 + (Ny2 - Ny1)^2}
\end{aligned}
\]

Find point given position along the line between nodes

\[
\begin{aligned}
px = Nx1 + x_{len} / tot_{len} \times I \\
py = Ny1 + y_{len} / tot_{len} \times I \\
p_i = (px, py)
\end{aligned}
\]

where $I$ is the increment, selected as 10m which increases by $I$ given $I < tot_{len}$


From a point $p_i$ at position $I$ between two nodes $n1n2$, to create a perpendicular sample, first points at a distance $\delta$ were created, with $\delta$ selected as 30m.

\[
\begin{aligned}
ref_{len} = \sqrt{(nx2 - pxi)^2 + (ny2 - pyi)^2} \\
xn_{len} = (\delta / ref_{len}) * (px2 - pxi) \\
yn_{len} = (\delta / ref_{len}) * (py2 - pyi)
\end{aligned}
\]
...
\noindent
where $I_k$ is the interval value which increases by 10 meters until the length of the total distance of the node, given $I_{1}, I_{2}, \dots, I_{k}>0$ and $k \geq 2$. Additionally 
...
\filldraw (-1,4) circle[radius=1pt] node [below left] {$B_2$};
\draw[-] (-1,4) -- (1,3);
\draw[dashed] (-1,3) -- node[fill=white,inner ysep=3pt, inner xsep = 3pt]{$O_B$} (1,3);
\draw[dotted] (-1,3) -- (-1,4);
\filldraw (1,3) circle[radius=1pt] node [above right] {$B_1$};
...
There is the possibility that the maximum distance between two points does not provide the maximum perpendicular distance across a road section, considering Pythagoras' theorem $A^2 + B^2 = C^2$, width filtering can provide a solution to this problem. Given the maximum opposite ($B$) line length between two points in a sample line is 2m, (each sample is 2m by 60m), for any line where $B$ is greater than $A$, $A$ must be below 2m and thus is removed during linestring filtering. 
