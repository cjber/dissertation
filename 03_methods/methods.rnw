...
For each road the maximum bearing angle between two nodes was selected, as well as the average bearing angle between two nodes.
...
\begin{itemize}
    ...
    \item \textbf{Road Angles:} The bearing angle change in road direction, considering the initial road direction a 0$^o$ (Section \ref{sec:angles})
\item \textbf{Road Elevation Change:}
\item \textbf{Road Surface Quality:}
\end{itemize}

The value obtained from this was called the Road Quality Index, presented in full on Table \ref{tab:rqi}.
