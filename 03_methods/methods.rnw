...
\[
\begin{aligned}
   \Delta e = \sum \frac{\Delta e(\mathbf{N}_i, \mathbf{N}_{i+1})}{L_i \times 1000},
   \end{aligned}
\]
\begin{flushright}
    \footnotesize{\ref{code:height_change}}
...
\noindent where $L_i$ is the length of a road $i$ in meters.
...
To find the angle $\theta_\mathbf{K}$, the difference in $x$ and $y$ coordinates between two nodes $\mathbf{N}_i$ and $\mathbf{N}_{i+1}$ was calculated as in Equation \ref{eq:xydiff} to obtain the bearing angle between these nodes, represented by the grey line on Figure \ref{fig:sample_points}. Similarly, the difference in $x$ and $y$ coordinates was found for the perpendicular points associated with the sample. With this, $\theta_\mathbf{K}$ given;

\[
\begin{aligned}
    \theta_\mathbf{K}=\mathrm{atan} 2\left(|x|_l,|y|_l\right) - \mathrm{atan} 2\left(|x|_s,|y|_s\right) - 45
\end{aligned}
\]
\begin{flushright}
...
\noindent where the difference in linestring coordinates gives $L = (|x|_l,|y|_l)$, and the difference in sample line coordinates gives $s = (|x|_s,|y|_s)$. Subtracting 45$^\circ$ gives the position relative to the perpendicular line ($B_\mathbf{K}$; Figure \ref{fig:sample_points}) rather than in the direction of the road centreline ($a_\mathbf{K}$).
