...
Then, to find a point ($p_i$) along these two nodes, at an interval distance $I_k$, with $I_1 = 10$ increasing incrementally by 10m, was achieved through this equation;
...
\noindent giving a point $p_k = (px,py)$ at a distance $I_k$ from $N_i$ in the direction of $N_{i+1}$. Where $I_k$ is the increment, which increases by 10m given $I_{1} = 10, I_{2} = 20, \dots, I_{k}>0$ and $k \geq 2$. To create a perpendicular sample from a point $p_k$ at position $I_k$ between two nodes $N_i$ and $N_{i+1}$, first two points at a perpendicular distance $\delta$ from the bearing angle of the two nodes, at a point $p_k$ were created, with $\delta$ selected as 30m. First the euclidean distance from $N_{i+1}$ to the point $p_i$ was calculated;
...
then to create perpendicular points at length $\delta$ from the point $p_i$ between the nodes the value $\delta_y$ was added to the $x$ value of the point $p_i$, while the value $\delta_y$ was subtracted from the $y$ value of the point $p_i$. This was then inverted to produce a second point. This may be expressed as;
