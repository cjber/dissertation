...
QGIS \citep{QGIS} used to measure roughly the width at various points along each road using 25cm resolution aerial imagery. With the widths, the results of each model was compared to assess model accuracy.Normalisation was achieved through finding the relative difference in width as a percentage;
