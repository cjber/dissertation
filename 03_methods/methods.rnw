...
\end{tikzpicture}
\caption{Bearing Angle between the first ($\mathbf{N}_1$) and second node ($\mathbf{N}_2$) giving $\hat \theta_1$ in relation to the north ($N$). Bearing angle for and third node ($\mathbf{N}_3$) giving $\hat \theta_2$ in relation to the bearing angle $\hat \theta_1$}
...
\begin{aligned}
    ...
    \overline{spz} \in  \left[A(\mathbf{N}_i)\times 1\right], \\
    \overline{spz} \in  \left[A(\mathbf{N}_{i+1})\times 1\right]
\end{array}\right.,
\end{aligned}
\]
\begin{flushright}
...
\noindent where $spi$ represents LiDAR point intensities with a single pulse return and ground classified, and $\Delta q(\mathbf{N}_1, \mathbf{N}_2, \dots, \mathbf{N}_i)$ represents the total change in road surface quality per road, measured by the range of intensity values.
