...
\[
    \begin{aligned}
\mathbf{c}_{k} = \Big(\mathbf{p}_i \in \Big[\frac{95}{100} \times \mathbf{p}_{i_{lpi}}\Big]\Big) \\
...
\noindent
where $\mathbf{c_k}$ represents a 10m$^2$ selection of LiDAR points, where each point $(\mathbf{p_i})$ has an intensity value within the 95\% percentile intensity for all points $(\mathbf{p}_{i_{lpi}})$ within $\mathbf{c_i}$.
...
B roads are considered to be classified roads, and other functions are considered to be unclassified. All roads are single carriageway, and so for the purpose of this analysis it is assumed they likely have the default national speed limit of 60mph. All private roads were removed, as were roads with a length of less than 50m, often those clipped by the extent of the LiDAR data.

\subsection{Road Sampling}

The LiDAR point cloud data was sampled at regular 10 meter intervals for each road, perpendicular to the road direction, ensuring that when road direction changed, the sampling locations remained perpendicular. To achieve this, each road was first split into nodes at which road direction changed, with a single road consisting of multiple nodes with coordinates $xy$, indicating a point along a road where the road direction changed. From this, points with $xy$ coordinates were created at 10 meter intervals beginning at the start of a road, (considered Node 1; $\mathbf{N}_1$), until the next node along the road ($\mathbf{N}_2$). To calculate these points along each line between two nodes ($\mathbf{N}_{i} = (x_i, y_i)$ and $\mathbf{N}_{i+1} = (x_{i+1},y_{i+1})$), first the individual change in $x$ and $y$ values was calculated, expressed as;
...
To find a point ($p_i$) along these two nodes, at an interval distance $I_k$, with $I_1 = 10$, was achieved through these equations;
...
\noindent where $r$ is the length of the line segment $AB$. Rearranging the equation for $\theta$ gives;
...
\noindent finally the bearing angle $\theta \in[0,2 \pi]$ is given as;
...
\noindent with the bearing angle of the first line segment ($\hat \theta_{AB}$) for a particular road, the change in orientation of the second road segment $BC$ may be given as $\theta_{BC} = \theta_{BC} - \theta_{AB}$, with additional nodes following the pattern $\theta_{n+1} = \theta_{n+1} - \theta_{n}$.
...
For each road the maximum bearing angle between two nodes was selected, as well as the average bearing angle between two nodes. Given the bearing angle between the first two nodes gives only the initial direction of the road, this was set to zero; $\hat \theta_{AB} = 0$.
