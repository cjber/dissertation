...
To find the angle $\theta_\mathbf{K}$, the difference in $x$ and $y$ coordinates between two nodes $\mathbf{N}_i$ and $\mathbf{N}_{i+1}$ was calculated as in Equation \ref{eq:xydiff} to obtain the bearing angle of the linestring associated with the sample. Similarly, the difference in $x$ and $y$ coordinates was found for the sample line. With this, $\theta_k$ was found by;

\[
\begin{aligned}
\theta=\mathrm{atan} 2\left(|x|_l,|y|_l\right) - \mathrm{atan} 2\left(|x|_l,|y|_l\right) - 45
\end{aligned}
\]

\noindent where the difference in linestring coordinates gives $L = (|x|_l,|y|)$, and the difference in sample line coordinates gives $s = (|x|_s,|y|_s)$. Subtracting 45$^\circ$ gives the position relative to the perpendicular line ($B_\mathbf{K}$; Figure \ref{fig:sample_points}) rather than in the diretion of the road centreline ($a_\mathbf{K}$).
\begin{flushright}
    \footnotesize{\ref{code:opposite_length}}
\end{flushright}
