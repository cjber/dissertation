...
\[
\begin{aligned}
   \textbf{p}_i \in \left[A(r_i) \times 30\right],
\end{aligned}
\]
\begin{flushright}
...
The angle of each bend in a road were identified through the nodes produced in the creation of the road shapefiles. First the road linestrings were split into points, with coordinates representing each node within a road, a point at which the orientation of the linestring is altered, (See Figure \ref{fig:bearingang1} \ref{fig:bearingang2} for illustrations of road nodes).
...
\[
    \begin{aligned}
\hat \theta=\left\{\begin{array}{ll}{\hat{\theta},} & {\hat{\theta} \geq 0} \\
...
With the bearing angle of the first line segment ($\hat \theta_{1}$) for a particular road, the change in orientation of the second  line segments between nodes $\mathbf{N}_2$ and $\mathbf{N}_3$ may be given;

\[
    \begin{aligned}
\hat{\theta}_2=& \mathrm{atan} 2\left(x_{2}-x_{1}, y_{2}-y_{1}\right) - 
& \mathrm{atan} 2\left(x_{3}-x_{2}, y_{2}-y_{2}\right) 
\end{aligned}
\]
\begin{flushright}
    \footnotesize{\ref{code:road_angles}}
...
as $\theta_{2} = \theta_{2} - \theta_{1}$, with additional nodes following the pattern $\theta_{i} = \theta_{i} - \theta_{i-1}$. Illustrated on Figure \ref{fig:bearingang2}.
...
\end{tikzpicture}
\caption{Bearing Angle between the first ($\mathbf{N}_1$) and second node ($\mathbf{N}_2$) giving $\hat \theta_1$, the second and third node ($\mathbf{N}_3$) giving $\hat \theta_2$ for a single road)}
