    ...
    Given in \textbf{Appendix \ref{a:code}} are the code snippets utilised in this methodology, for many equations, the relevant code is given as a reference to the appendix location, in the form \textbf{A.x.x}. Due to the nature of the functions used in this analysis, a single function often contains multiple equations, and so a reference to a particular appendix number may be repeated. Additionally, not all functions are referenced in text, so there are gaps in references to functions, and they do not necessarily appear sequentially.
...
The LiDAR point cloud data used in this paper gives the values for 5 pulse returns. The canopy above roads may be excluded through ignoring early pulses (higher Z values), therefore only the last pulse values for any point are selected, taking only points where \textbf{ReturnNumber} equals the \textbf{NumberOfReturns};
...
Intensity noise was filtered through area based outlier detection, measuring the 95th percentile values within a 10m$^2$ area, removing all points above the 95\% percentile. This can be expressed as;
...
Next, the preprocessed LiDAR data was combined with the OS road shapefiles and aerial imagery to obtain a set of criteria to assess the chosen road network. A summary of the information provided alongside OS road shapefiles is given on Table \ref{tab:roadstab}. While both \texttt{roadNumberTOID} and \texttt{roadNameTOID} do provide true road identification for many roads, this was not true for each road in the area chosen. Due to this, it was impossible to identify what could be considered an individual road, meaning a "road" will now be defined as indicated on Figure \ref{fig:area_map}, selected based on the shapefile geometry provided.
...
The bend in a road was considered to be the \textit{bearing angle} $\hat \theta$, from a node $\mathbf{N}_i = \left(x_{i}, y_{i}\right)$ to a node $\mathbf{N}_{i+1} = \left(x_{i+1}, y_{i+1}\right)$, with the angle measured in a clockwise direction from north. This is represented on Figure \ref{fig:bearingang1}.
...
\draw[-] (0.2,.4) -- (0.8, .7);
    ...
    \node[] at (.25,.5)  {$\hat \theta$};
\end{tikzpicture}
\caption{Bearing Angle ($\theta$) between two sequential road Nodes, $\mathbf{N}_{i}$ and $\mathbf{N}_{i+1}$, North is given by $N$}\label{fig:bearingang1}
\end{figure}

To find the angle $\theta$, the node $\mathbf{N}_{i+1}$ can be represented into relation to node $\mathbf{N}_{i}$ as;
...
$$
\hat{\theta}=\mathrm{atan} 2\left(x_{i+1}-x_{i}, y_{i+1}-y_{i}\right) \in[-\pi, \pi]
$$
...
With the bearing angle of the first line segment ($\hat \theta_{_1}$) for a particular road, the change in orientation of the second  line segments between nodes $\mathbf{N}_2$ and $\mathbf{N}_3$ may be given as $\theta_{2} = \theta_{2} - \theta_{1}$, with additional nodes following the pattern $\theta_{i} = \theta_{i} - \theta_{i-1}$.
...
\end{figure}
 ...
 Given the bearing angle between the first two nodes gives only the initial direction of the road, this was set to zero; $\hat \theta_{1} = 0$. In the final analysis, for each road the maximum bearing angle between any two nodes was selected, as well as the average bearing angle between all nodes.
...
\[
\begin{aligned}
    \Delta e(\mathbf{N}_i, \mathbf{N}_{i+1}) = \Delta \left(\overline{lpz} \in  A(\mathbf{N}_i)\times 1, \overline{lpz} \in  A(\mathbf{N}_{i+1})\times 1\right),
\end{aligned}
\]
\begin{flushright}
...
\noindent where $\Delta e(\mathbf{N}_i, \mathbf{N}_{i+1})$ is the change in elevation between sequential nodes $\mathbf{N}_i$ and $\mathbf{N}_{i+1}$, taking the change in mean last pulse values ($\overline{lpz}$) within a 1m$^2$ buffer of each node.

\section{Surface Quality}\label{sec:qual}

Surface quality was assessed roughly through the range in intensity values found at a 1m$^2$ area around each road node. To ensure there was no inaccuracy in intensity values caused by later returns passing through a canopy, only points that had a single return pulse were used in this analysis;

\[
\begin{aligned}
    \Delta q(\mathbf{N}_1,\mathbf{N}_2,\dots, \mathbf{N}_{i}) = \Delta (\overline{spi}) \in  A(\mathbf{N}_i)\times 1,
\end{aligned}
\]
\begin{flushright}
    \footnotesize{\ref{code:surface_qual}}
\end{flushright}

\noindent where $spi$ represents LiDAR point intensities with a single pulse return, and $\Delta q(\mathbf{N}_1, \mathbf{N}_2, \dots, \mathbf{N}_i)$ represents the total change in road surface quality per road, measured by the range of intensity values.

\subsection{Road Sampling}

The LiDAR point cloud data was sampled at regular 10 meter intervals for each road, perpendicular to the road direction, ensuring that when road direction changed, the sampling locations remained perpendicular. To achieve this, each road was first split into nodes at which road direction changed, with a single road consisting of multiple nodes with $xy$ coordinates, indicating a point along a road where the road direction changed. From this, points with $xy$ coordinates were created at 10 meter intervals beginning at the start of a road, (considered Node 1; $\mathbf{N}_1$), until the next node along the road ($\mathbf{N}_2$). To calculate these points along each line between two nodes ($\mathbf{N}_{i} = (x_i, y_i)$ and $\mathbf{N}_{i+1} = (x_{i+1},y_{i+1})$), first the individual change in $x$ and $y$ values was calculated, expressed as;
...
\noindent then to create perpendicular points at length $\delta$ from the point $p_i$, the value $\delta_y$ was added to the $x$ value of the point $p_i$, while the value $\delta_y$ was subtracted from the $y$ value of the point $p_i$. This was then inverted to produce a second point. This may be expressed as;
...
\section{Linear Probability Models and Road Width}\label{sec:lpm}
...
\draw[dashed, very thick, color = gray] (0,0.3) -- (0,4.2);
...
To aid with model interpretability, the direct comparison between each variable in the analysis was enabled through centering and scaling through the use of beta coefficients \citep{peterson2005};
...
\begin{itemize}
    \item \textbf{Road Width:} Extracted through linear probability classification of the road surface using LiDAR data (Section \ref{sec:lpm})
