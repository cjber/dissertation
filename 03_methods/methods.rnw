...
\noindent
where $\mathbf{c}_k$ represents a 10m$^2$ selection of LiDAR points, where each point $(\mathbf{p_i})$ has an intensity value within the 95\% percentile intensity for all points $(\mathbf{p}_{i_{lpi}})$ within $\mathbf{c}_k$.
...
The bend in a road was considered to be the \textit{bearing angle} $\theta$, from a node $\mathbf{N}_i = \left(x_{i}, y_{i}\right)$ to a node $\mathbf{N}_{i+1} = \left(x_{i+1}, y_{i+1}\right)$, with the angle measured in a clockwise direction from north. This is represented on Figure \ref{fig:bearingang1}.
...
\draw[-] (0.2,.4) -- (0.8, .7);
    ...
    \node[] at (.25,.5)  {$\theta$};
\end{tikzpicture}
\caption{Bearing Angle Between Road Nodes}\label{fig:bearingang1}
\end{figure}

To find the angle $\theta$, the node $N_{i+1}$ can be represented into relation to point $N_{i}$ as;

$$
\left(i+1_{x}, i+1_{y}\right)=\left(i_{x}+r \sin \theta, i_{y}+r \cos \theta\right)
$$
