...
<<>>=
final_data <- fread("/home/cjber/drive/uni/envs492/main/data/final_data/final.csv") %>%
  select(-c(V1, roadFunction, meanAngle, meanZ, meanInt)) %>%
...
mean((filter_las$lm1_dum == 1) == (filter_las$road == 1))
mean((filter_las$lm2_dum == 1) == (filter_las$road == 1))
mean((filter_las$lm3_dum == 1) == (filter_las$road == 1))
mean((filter_las$glm1_dum == 1) == (filter_las$road == 1))
...
<<lm_figs>>=
ggRGB(aerial) +
    geom_sf(data = road_lm_filter, colour = "black", alpha = 0.1) +
    geom_sf(
