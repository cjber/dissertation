...
centrelines_buff <- centrelines %>% st_buffer(30)

las <- lasclip(las, extent(centrelines_buff))
aerial <- raster::crop(aerial, extent(centrelines_buff))
...
roads <- st_crop(centrelines_buff, centrelines)

chm_gg <- ggplot() +
  geom_raster(data = chm, aes(x = x, y = y, fill = value), alpha = 1) +
  geom_sf(data = centrelines_buff, colour = alpha("red", .3), fill = "black", alpha = 0.1) +
  scale_fill_viridis() +
  coord_sf() +
  theme_map() +
