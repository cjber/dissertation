...
<<fig_noisecent, fig.cap = "Comparison between Linear Probability models applied to; (A) OS Road Centrelines, and (B) Derived Centrelines. Green points give classified road locations, red show classified road locations removed through isolation based filtering", fig.showtext=TRUE, fig.pos = "b">>=
plot_grid(
  ggcent,
  gg_cent0,
  gg_cent1,
...
cat(c("\\begin{table}[!b]
    ...
    \\caption{Model Coefficients, Comparison between Linear Probability Model 1 and 2}\\label{tab:coeftest}
      \\centering",
        t1,
    "\\end{minipage}%
    \\begin{minipage}{.5\\linewidth}
      \\centering
    \\caption{Comparison between different linear probability models for each individual road, in relation to estimated true road width}\\label{tab:estwidths}",
        t2,
    "\\end{minipage} 
\\end{table}"
))  
...
Given Linear Model 2 with corrected road centrelines appears to produce the most accurate estimates for road widths, this was used in the final road assessment. Table \ref{tab:rqi} gives the full results of the road geometric extraction, along with an estimate of overall road quality given by the Road Quality Index (RQI). Qualitative assessment of the RQI may be achieved through observation of the highest and lowest values (Figure \ref{fig:rqi_compare}). It appears to produce reliable results, as the road with the highest RQI is straight, wider than the road with the lowest RQI, and is likely flat given it is neighboured by two agricultural fields. Additionally, this figure gives both the original, and derived centrelines for both roads. Notably the reduction in accuracy in the centreline for road 14 has greatly reduced the accuracy of the road width estimate (Table \ref{tab:estwidths})
...
rd_tc <- cents[cents$road_id == "road_34", ] %>% 
    st_crop(rd_t)
rd_b <- roads[roads$road_id == "road_14", ]
rd_bc <- cents[cents$road_id == "road_14", ] %>% 
    st_crop(rd_b)
...
gg_ta <- ggRGB(ta) +
    geom_sf(data = rd_t, colour = "red") +
    geom_sf(data = rd_tc, colour = "green") +
  theme_map() +
  theme(panel.border = element_rect(
    colour = "black",
    fill = NA, size = 1
  ...
  scale_y_continuous(expand = c(0, 0))
gg_ba <- ggRGB(ba) +
    geom_sf(data = rd_b, colour = "red") +
    geom_sf(data = rd_bc, colour = "green") +
  theme_map() +
  theme(panel.border = element_rect(
    colour = "black",
    fill = NA, size = 1
  ...
  scale_y_continuous(expand = c(0, 0))
@
<<rqi_compare, fig.cap = "Visual comparison between road with the highest RQI \\textbf{(A)}, and lowest RQI \\textbf{(B)}, original centrelines are given in red, with derived centrelines given in green", fig.showtext=TRUE>>=
plot_grid(
          gg_ta,
          gg_ba,
  labels = "AUTO",
