...
Table \ref{tab:lidartab} indicates that there are likely some points with noise, particularly reflected by the highest intensity value ($\Sexpr{max(lidar_tab$Intensity)}$) relative to its standard deviation ($\Sexpr{round(sd(lidar_tab$Intensity),0)}$), with 99\% of observations within the range of 0 to \Sexpr{quantile(lidar_tab$Intensity, .99)}. As noted in previous LiDAR classification methods, intensity is often subject to noise, therefore a simplistic noise exclusion technique \citep{lidR} was considered, as described in Section \ref{subsec:noise}.
