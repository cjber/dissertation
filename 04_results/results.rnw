...
Given Linear Model 2 with corrected road centrelines appears to produce the most accurate estimates for road widths, this was used in the final road assessment. Table \ref{tab:rqi} gives the full results of the road geometric extraction, along with an estimate of overall road quality given the Road Quality Index (RQI). Qualitative assessment of the RQI may be achieved through observation of the highest and lowest values (Figure \ref{fig:rqi_compare}).
...
rd_t <- roads[roads$road_id == summary_data$road_id[summary_data$qual_index == max(summary_data$qual_index)]]
rd_b <- roads[roads$road_id == summary_data$road_id[summary_data$qual_index == min(summary_data$qual_index)]
