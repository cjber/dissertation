...
<<hist_lidar, fig.cap="LiDAR point cloud intensity distribution, post noise filtering", fig.showtext=TRUE, fig.height = 3>>=
...
gg_ba <- ggRGB(ba) +
  ...
  scale_y_continuous(expand = c(0, 0))
@
<<rqi_compare, fig.cap = "Visual comparison between road with the highest RQI \\textbf{(A)}, and lowest RQI \\textbf{(B)}">>=
plot_grid(
          gg_ta,
          gg_ba,
  labels = "AUTO",
