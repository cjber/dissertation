...
gg_ea <- ggRGB(ea) +
  ...
  scale_y_continuous(expand = c(0, 0))
@
<<rqi_compare, fig.cap = "Visual comparison between the RQI of roads. \\textbf{(A)} highest RQI , \\textbf{(B)} lowest RQI \\textbf{(C)} and Road 14, showing inaccuracy in derived centrelines, with original centrelines are given in red, with derived centrelines given in green.", fig.scap = "Visual comparison between the RQI of roads.", fig.showtext=TRUE>>=
plot_grid(
          gg_ta,
          gg_ba,
          gg_ea,
