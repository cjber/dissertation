...
centrelines <- read_sf("/home/cjber/drive/uni/envs492/main/data/derived/roads/roads_line.gpkg", quiet = TRUE)

rd_f <- "road_6"
centrelines <- centrelines[centrelines$road_id == rd_f, ] %>% 
    st_cast("LINESTRING")
centrelines <- centrelines[4, ] %>% 
    st_buffer(30)

las <- lasclip(las, extent(centrelines))
aerial <- raster::crop(aerial, extent(centrelines))
...
roads <- read_sf("/home/cjber/drive/uni/envs492/main/data/derived/roads/roads_line.gpkg", quiet = TRUE)
roads_buff <- roads_buff[roads_buff$road_id == "road_6", ]
roads <- st_crop(roads_buff, roads)
...
<<fig_lp, fig.cap = "LiDAR point clouds for one selected road section aggregated into 2m$^2$ grids, \\textbf{(A)} Base point cloud $z$ values, road location indicated with a 30m buffer \\textbf{(B)} Normalised Point cloud $z$ values for only last returns ($lpz$) \\textbf{(C)} Normalised Point cloud $Intensity$ values for last returns, \\textbf{(D)} Aerial Data combined to 1 band", fig.showtext=TRUE>>=
plot_grid(
  chm_gg,
  dsm_lp_gg,
  las_i_gg,
...
# example road (partial section of road 6)
centrelines <- centrelines[centrelines$road_id == rd_f, ]
centrelines <- centrelines %>% st_cast("LINESTRING")
centrelines <- centrelines[4, ]

sample_index <- st_intersects(sample_lines, centrelines, sparse = F)
sample_lines <- sample_lines[sample_index, ]

road_lm <- st_crop(road_lm, st_bbox(sample_lines))
...
road_lm1 <- road_lm[road_lm$lm1_dum == 1, ] %>% distinct(.keep_all = T)
road_lm90 <- road_lm[road_lm$lm1_dum90 == 1, ] %>% distinct(.keep_all = T)
road_lm80 <- road_lm[road_lm$lm1_dum80 == 1, ] %>% distinct(.keep_all = T)

q95 <- ggRGB(aerial) +
  geom_sf(data = road_lm1, colour = "green") +
  geom_sf(data = centrelines, colour = "red") +
  theme_map() +
  theme(panel.border = element_rect(
    colour = "black",
    fill = NA, size = 1
...
q90 <- ggRGB(aerial) +
  geom_sf(data = road_lm90[road_lm90$road_id == rd_f, ], colour = "green", size = 0.1, alpha = 0.1) +
  theme_map() +
  theme(panel.border = element_rect(
    colour = "black",
    fill = NA, size = 1
...
q80 <- ggRGB(aerial) +
  geom_sf(data = road_lm80[road_lm80$road_id == rd_f, ], colour = "green", size = 0.1, alpha = 0.1) +
  theme_map() +
  theme(panel.border = element_rect(
    colour = "black",
    fill = NA, size = 1
...
<<quant_fig, fig.cap = "Comparison between Linear Prediction Quantiles; \\textbf{(A)} 95th Quantile, \\textbf{(B)} 90th Quantile, \\textbf{(C)} 80th Quantile", fig.showtext=TRUE>>=
plot_grid(
  q95,
  q90,
  q80,
...
Figure \ref{fig:quant_fig} reveals that qualitatively, the optimal choice for a quantile filtering of the linear probability distribution is likely the 95th quantile (Figure \ref{fig:quant_fig} \textbf{(A)}). However, observation of the southern section of Figure \ref{fig:quant_fig} \textbf{(A)} reveals that inaccurate centreline locations have led to an incomplete linear model analysis. To compensate for this, a further method proposed aims to improve the accuracy of the given road centreline locations. Additionally, Figure \ref{fig:lmdistributions} reveals that for the 95th quantile probability values, shadow from road hedgerows appears to reduce the model accuracy, as noticeable towards the top end of the road. For this reason, a second model was constructed for later comparison, which removes the $luminescence$ information provided by the aerial imagery.
...
<<fig_noisecent, fig.cap = "Comparison between Linear Probability models applied to; (A) OS Road Centrelines, and (B) Derived Centrelines. Green points give classified road locations, red show classified road locations removed through isolation based filtering", fig.showtext=TRUE>>=
plot_grid(
  ggcent,
  gg_cent0,
  gg_cent1,
...
make_table(coefs,
  cap = "Model Coefficients, Comparison between Linear Probability Model 1 and 2")
@
...
@
<<estwidths, fig.cap = "Comparison between different Linear Probability Models and the estimated road width", results = 'asis'>>=
...
rd_t <- roads[roads$road_id == "road_34", ]
rd_b <- roads[roads$road_id == "road_14", ]

ta <- lapply(jpgs, function(x) {
  return(tryCatch(crop(x, rd_t), error = function(e) NULL))
})
ta <- ta %>%
  compact() %>%
  brick()
ba <- lapply(jpgs, function(x) {
  return(tryCatch(crop(x, rd_b), error = function(e) NULL))
})
ba <- ba %>%
  compact() %>%
  brick()
...
gg_ba <- ggRGB(ba) +
  ...
  scale_y_continuous(expand = c(0, 0))
@
<<rqi_compare, fig.cap = "Visual comparison between road with the highest RQI \\textbf{(A)}, and lowest RQI \\textbf{(B)}", fig.showtext=TRUE>>=
plot_grid(
          gg_ta,
          gg_ba,
  labels = "AUTO",
