...
<<lmdistributions>>=
fread("../data/derived/model_data/linearmodels.csv")
##### plot scatter of pred dists
@
...
glm1 <- mean((sampled_las$glm1_dum == 1) == (sampled_las$road == 1))

# filter out  > 0.05 p values for each point
lmi <- mean((filter_las$lmI_dum == 1) == (filter_las$road == 1))

lm_assess <- rbind(lm1, lm90, glm1) %>%
  as.data.frame()
@
<<>>=
