...
<<quant_fig, fig.cap = "Comparison between Linear Prediction Quantiles showing an example road segment", showtext=TRUE>>=
plot_grid(
  q95,
  q90,
  q80,
...
<<fig_noisecent, fig.cap = "Comparison between Linear Probability models applied to; (A) OS Road Centrelines, and (B) Derived Centrelines. Green points give classified road locations, red show classified road locations removed through isolation based filtering", showtext=TRUE>>=
plot_grid(
  ggcent,
  gg_cent0,
  gg_cent1,
...
gg_ba <- ggRGB(ba) +
  ...
  scale_y_continuous(expand = c(0, 0))
@
<<rqi_compare, fig.cap = "Visual comparison between road with the highest RQI \\textbf{(A)}, and lowest RQI \\textbf{(B)}", showtext=TRUE>>=
plot_grid(
          gg_ta,
          gg_ba,
  labels = "AUTO",
