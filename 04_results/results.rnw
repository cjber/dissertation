...
For the purpose of this assessment, the final model produced (LM 2) was used in the final road assessment, despite the slightly lower model accuracy than the original centrelines model. Table \ref{tab:rqi} gives the full results of the road geometric extraction, along with an estimate of overall road quality given by the Road Quality Index (RQI). Qualitative assessment of the RQI may be achieved through observation of the highest and lowest values (Figure \ref{fig:rqi_compare}). It appears to produce reliable results, as the road with the highest RQI is straight, wider than the road with the lowest RQI, and is likely flat given it is neighboured by two agricultural fields. Additionally, this figure gives both the original, and derived centrelines for both roads. Notably the reduction in accuracy in the centreline for road $\Sexpr{tail(summary_data$road_id, 1)}$ has greatly reduced the accuracy of the road width estimate (Table \ref{tab:estwidths}).
...
bbox_e <- bbox[bbox$road_id == "road_14", ]
...
ea <- lapply(jpgs, function(x) {
  return(tryCatch(crop(x, bbox_e), error = function(e) NULL))
})
ea <- ea %>%
  compact() %>%
  brick()

rd_e <- roads[roads$road_id == "road_14", ] %>% 
    st_crop(ea)
rd_ec <- cents[cents$road_id == "road_14", ] %>% 
    st_crop(ea)
...
gg_ea <- ggRGB(ea) +
    geom_sf(data = rd_e, colour = "red") +
    geom_sf(data = rd_ec, colour = "green") +
  theme_map() +
  theme(panel.border = element_rect(
    colour = "black",
    fill = NA, size = 1
  ...
  scale_y_continuous(expand = c(0, 0))
@
<<rqi_compare, fig.cap = "Visual comparison between road with the \\textbf{(A)} highest RQI , \\textbf{(B)} lowest RQI \\textbf{(C)} and Road 14, showing inaccuracy in derived centrelines, with original centrelines are given in red, with derived centrelines given in green.", fig.showtext=TRUE>>=
plot_grid(
          gg_ta,
          gg_ba,
          gg_ea,
