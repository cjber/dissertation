...
While considered a supervised method, the \textit{training} data used is derived from the preexisting OS road centrelines, and as such may be recreated automatically for any road in England, given the comprehensive coverage of this data \citep{ordnancesurvey2019a}. This differs from existing supervised road classification methods, where a training dataset is created and manually labelled \citep[][e.g.]{charaniya2004}. Additionally, the majority of LiDAR road classification techniques focus on unsupervised methods \citep[e.g.][]{clode2004,vosselman2009,jaakkola2008,darmawati2008}, and as such do not consider the use of any training data, with the primary goal of obtaining accurate road locations. Such techniques often limit their scope to road centreline extraction, notably, \cite{zhang2018} assess the accuracy of their technique by comparing derived road centrelines to an existing road centreline database}. The method presented in this dissertation considers a more practical approach, providing an intermediate between unsupervised and supervised methods, which integrates the known road centrelines into the road classification.

Additionally, rather than attempting to classify road centrelines, or the full road surface, this dissertation concentrates on the requirement for road feature extraction. With the primary focus of road classification on the automated road width extraction. This enables use of sample lines in this analysis, which primarily enables a reduction in computational overhead, while adding the benefit of more simplistic noise filtering techniques.
