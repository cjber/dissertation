...
At present the removal of noise at the final stage of the road classification comes from both identifying isolated points (See Function \ref{code:max_lines}), and the removal of calculated widths that are above 8m and below 2m \ref{c:script}. While logically it makes sense to include limitations for widths, given a road below 2m would not support even single way traffic, and a 8m road is unexpected for any rural single carriageway, these limitations are still arbitrary, and for all unclassified roads in England, there is no minimum width required \citep{highwaysengland2016}.
