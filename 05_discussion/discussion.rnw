...
@
    An assumption is made that roads are a constant width, however, as the roads observed in this study were relatively short, with an average road length of \Sexpr{round(mean(roads$len),2)}, it is unlikely that width changes significantly over this distance. The current inaccuracy of this method however relies on the aggregation of road sampled widths to provide an overall mean for a road, and as such is unable to provide accurate information regarding a road at any given point.
