...
<<area_map, fig.cap="Study area highlighting road centrelines outlined in a lighter 30m buffer", fig.pos = "t">>=
...
ggRGB(aerial) +
  geom_sf(data = rd, fill = "white", colour = "white", alpha = 0.2) +
...
\begin{itemize}
    ...
    \item Using extracted road features, outline the overall quality of the road network, allow for direct comparison between each road.
\end{itemize}
