...
<<area_map, fig.cap="Study area highlighting road centrelines, each colour represents a seperate 'road' as defined by the OS Data provided. Thicker roads are B roads, while thinner are Unclassified. The inset map gives shows an example road, with features typical of roads within this study area.", fig.pos = "t", cache = T, autodep = T>>=
...
jpgs <- Sys.glob("../data/aerial/*.jpg")
jpgs <- lapply(jpgs, brick)
aerial_inset <- lapply(jpgs, function(x) {
  return(tryCatch(crop(x, inset_rd), error = function(e) NULL))
})

aerial_inset <- aerial_inset %>%
  compact() %>%
  brick()
...
main <- ggRGB(aerial) +
  ...
  geom_sf(data = rd_inset %>% st_bbox() %>% st_as_sfc(), colour = "red", fill = NA, size = 1) +
  theme_map() +
  theme(panel.border = element_rect(
    colour = "black",
    fill = NA, size = 1
  ...
  ggdraw() +
      draw_plot(main) +
      draw_plot(inset, x = 0, y = .35, width = .6, height = .6)
@
