...
ext <- extent(aerial_inset)
inset_shadow <- ggplot() +
  geom_rect(aes(xmin = ext[1] - 1, xmax = ext[2] + 1, ymin = ext[3] - 1, ymax = ext[4] + 1), fill = "black", alpha = .4) +
  theme_map() +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0))
...
ggdraw() +
  draw_plot(main) +
  draw_plot(inset_shadow, x = -.1, y = .35, width = .6, height = .6) +
