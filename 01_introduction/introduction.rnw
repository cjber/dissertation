...
<<area_map, fig.cap="Study area highlighting road centrelines, each colour represents a seperate 'road' as defined by the OS Data provided. Thicker roads are B roads, while thinner are Unclassified", fig.pos = "t", cache = T, autodep = T>>=
...
inset <- ggRGB(aerial_inset) +
  geom_sf(data = rd_inset, colour = "white")

main <- ggRGB(aerial) +
  geom_sf(data = rd_l, colour = cols, size = roadFunction) +
  geom_sf(data = rd_inset %>% st_bbox() %>% st_sf(), colour = cols, size = roadFunction) +
  theme_map() +
