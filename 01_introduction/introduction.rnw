...
<<area_map, fig.cap="Study area highlighting road centrelines; each colour represents a seperate 'road' as defined by the OS Data provided. Thicker roads are B roads, while thinner are Unclassified. The inset map gives shows an example road, with features typical of roads within this study area.", fig.pos = "b" >>=
ggdraw() +
  draw_plot(main) +
