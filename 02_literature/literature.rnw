...
Additionally \cite{taylor2002} categorised these roads into four key groups:
...
\begin{itemize}
    \item \textit{Initial State} A simulated 'cloth' is placed above the inverted LiDAR measurements. A series of points that lie flat to the surface and are allowed to move based on the influence of gravity.
