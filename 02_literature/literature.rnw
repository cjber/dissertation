...
Vegetation in particular exhibits multiple returns, whereas most man made surface objects do not, by considering the number of returns and homogenetic height variation \cite{filin2006} were able to categorise surface vegetation into both high vegetation, low vegetation and smooth man made surfaces.

\section{Road Classification}

// For additional information look at \cite{hatger2005}..
...
Many recent road extraction techniques have relied on kerb extraction or the detection of buildings, features uncommon to rural roads in the UK, where non road surfaces often include managed grass verges, with little height difference in comparison to roads \citep{kumar2013}. Rural classification therefore must rely on alternative features of roads, notably the difference in intensity produced by vegetation, explored in various studies // refs. Additionally, it appears uncommon the overhanging tree canopies are common features of roads that have been previously classified, and as such a methodology for classifying rural british roads must take the detection of this feature into consideration.
...
\begin{itemize}
    \item \textbf{Width:} Narrower roads are associated with an increased number of accidents in many studies of rural roads \citep{taylor2002,aarts2006}
...
// add more to outline the paper here.
